...
def calculate_custom_kernel(df):
...
def calculate_esm_kernel(df, model_name="facebook/esm2_t12_35M_UR50D"):
...
sim_custom, df_custom = calculate_custom_kernel(df_main.copy())
sim_esm, df_esm = calculate_esm_kernel(df_main.copy())
...
print("-> Aligning Custom Kernel plot onto ESM benchmark plot...")
...
df_esm_aligned = pd.DataFrame({
    'x': mtx1[:, 0], 'y': mtx1[:, 1], 'method': 'ESM (Reference)',
...
fig = px.scatter(
    ...
    hover_data=['h3_chain', 'pdb_id'], title="Procrustes-Aligned Kernel PCA Projections"
)
...
output_filename = "kpca_vs_kpca_comparison_v2.html"
